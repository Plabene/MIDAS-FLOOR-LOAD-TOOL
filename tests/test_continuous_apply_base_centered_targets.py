...
def test_base_centered_targets_include_connected_above_and_below():
...
def test_base_centered_targets_keep_all_applicable_stories_even_when_one_side_has_gap():
    app = object.__new__(FloorLoadAutoApp)
    candidates = [
        _candidate("1F", True),
        _candidate("2F", False),
    ...
    assert targets == ("1F", "4F", "5F")


def test_auto_select_uses_all_base_centered_targets_and_saves():
    app, _tree = _app_with_continuous_tree()
    ...
    assert app.continuous_tree.values["i1"][0] == "✓"
    assert app.continuous_tree.values["i3"][5] == "불가"
    assert "cannot_apply" in app.continuous_tree.tags["i3"]
...
def _app_with_continuous_tree():
    ...
    return app, app.continuous_tree
...
class _Tree:
    def __init__(self, iids):
        self.values = {iid: ("", iid, "", "", "", "가능" if iid != "i3" else "불가", "") for iid in iids}
        self.tags = {iid: ("can_apply",) if iid != "i3" else ("cannot_apply",) for iid in iids}
        self.selected = set()
